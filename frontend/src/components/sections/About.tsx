      ...
      <Container>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4 auto-rows-[150px] md:auto-rows-[150px]">
          ...
          <BentoCard delay={0.7} className={`md:col-span-3 lg:col-span-4 md:row-span-1 flex flex-col sm:flex-row justify-between items-center gap-6 cursor-pointer transition-colors ${hustleMode ? 'bg-[#111] border-white/10 hover:bg-white/5' : 'bg-white hover:bg-hairline/50'}`} onClick={fireConfetti}>
            ...
            <a href="/Niraj_Kushwaha__CV.pdf" download="Niraj_Kushwaha_CV.pdf" className="flex-shrink-0 z-10 select-none block cursor-pointer" draggable={false} onClick={(e) => { e.stopPropagation(); fireConfetti(); }}>
              <MagneticButton strength={0.2} className={`group relative overflow-hidden rounded-full px-8 h-12 flex items-center justify-center font-medium shadow-xl hover:shadow-2xl transition-all select-none cursor-pointer ${hustleMode ? 'bg-white text-black' : 'bg-ink text-white'}`}>
